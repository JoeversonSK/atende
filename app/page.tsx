...
function toMessage(value: Record<string, unknown>): MessageWithTimestamp {
  ...
  return { id: String(value.waMessageId || value.id || value.messageId || eventId()), body: String(value.body || value.text || value.content || fallback), mine: Boolean(value.fromMe) || String(value.direction).toLowerCase() === "outgoing", time: date && !Number.isNaN(date.valueOf()) ? date.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" }) : "", timestamp, type, media };
}
...
export default function Home() {
  ...
  async function finishTicket() {
    if (!selected || closingTicket) return;
