...
export class EventsGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect, OnModuleDestroy {
  ...
  private sweepExpiredApiKeys(now = Date.now()): void {
    for (const [keyId, sockets] of Array.from(this.socketsByKeyId.entries())) {
      for (const client of sockets) {
        const raw = client.data?.rawApiKey as string | undefined;
        if (raw?.startsWith('atende_')) void this.authService.validateApiKey(raw).catch(() => client.disconnect(true));
      }
  ...
  handleConnection(client: Socket) {
    ...
    client.data.atendeAuthentication = authentication;
  ...
  async handleMessage(@ConnectedSocket() client: Socket, @MessageBody() message: WSClientMessage) {
    await client.data.atendeAuthentication;
    if (!client.data.apiKey) return this.createError('UNAUTHORIZED', 'Authentication required');
