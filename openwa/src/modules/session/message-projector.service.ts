...
import { ContactProfileService } from '../operator-auth/contact-profile.controller';
...
export class MessageProjector {
  ...
  private async projectInboundMessage(
    ...
    if (!outcome) return;
    if (outcome.persisted && this.contactProfiles) {
      try {
        await this.contactProfiles.reopenOnIncoming(id, incoming.chatId, Number(incoming.timestamp), incoming.fromMe, incoming.isGroup);
      } catch (err) {
