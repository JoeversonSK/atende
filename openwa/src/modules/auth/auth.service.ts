...
export class AuthService implements OnModuleInit, OnModuleDestroy {
  ...
  async validateApiKey(rawKey: string, clientIp?: string, sessionId?: string): Promise<ApiKey> {
    if (process.env.ATENDE_REQUIRE_OPERATOR === 'true' && rawKey?.startsWith('atende_')) {
      const context = await this.moduleRef.get(OperatorAuthService, { strict: false }).connectionContext(rawKey.slice(7));
      if (sessionId && sessionId !== context.sessionId) throw new UnauthorizedException('Sessão não autorizada.');
      return Object.assign(new ApiKey(), { id: context.user.id, name: context.user.displayName,
        role: ApiKeyRole.OPERATOR, allowedSessions: [context.sessionId], allowedIps: null,
        isActive: true, expiresAt: context.expiresAt, keyHash: this.hashKey(rawKey), usageCount: 0 });
    }
